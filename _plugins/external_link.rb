...
module Jekyll
  class ExternalLinkTag < Liquid::Tag
    ...
    def render(context)
      ...
      return build_html(link_text, link_href, link_lang)
    end
    ...
    def build_html(text, href, custom_lang)
      ...
      html = ""
      html += %Q(<a href="#{href}" target="_blank" rel="noopener">)
      html += text
      html += %Q( (#{available_translations[lang]} <i class="fa fa-window-restore" aria-hidden="true"></i>))
      html += "</a>"
